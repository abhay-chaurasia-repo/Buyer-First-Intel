...
type PageHeaderProps = {
  /** Primary heading — cream night-ink, same as welcome Due Diligence */
  title: string
  description?: string
  testId?: string
  className?: string
  ...
  return (
    ...
    >
      <div className="animate-bfi-rise px-5 pb-2 pt-3 text-center" data-testid="page-title-open">
        <h1 className="mx-auto max-w-[20rem] font-display text-[1.35rem] font-semibold leading-snug tracking-tight text-night-ink">
          {title}
        </h1>
        {description ? (
          <p className="mx-auto mt-2 max-w-[20rem] text-[0.9rem] leading-relaxed text-night-ink">
            {description}
          </p>
        ) : null}
        {children}
